...
def learn_route():
	...
	try:
		...
		if request.method == 'POST':
			...
			# Add keyword knowledge.
			elif form['op'] == 'add_db':			
				keyword_data = form['keyword']
				image_type = 'image'
				os.system("python /home/badrzahir/Desktop/lucida-populate-db.py " + keyword_data)
				path = "/home/badrzahir/Desktop/"+keyword_data
				dirs = os.listdir(path)
				for image in dirs:
					im = image
					print im
					upload_file = open("/home/badrzahir/Desktop/"+keyword_data+"/"+image, 'r')
					#if upload_file.filename == '':
					#	raise RuntimeError('Empty file is not allowed')
					# Generate the id.
					image_data = upload_file.read()
					...
					thrift_client.learn_image(username, image_type, image_data, image_id)
    					database.add_image(username, image_data, keyword_data, image_id)
				os.system("rm -Rf /home/badrzahir/Desktop/"+keyword_data)
